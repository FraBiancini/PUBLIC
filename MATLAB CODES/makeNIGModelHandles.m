function model = makeNIGModelHandles(Dt, F0, B)
...
% Uses your Lewis/FFT pricing function 'PriceCallOption' and sampler 'SimulateFromCF'.
    ...
    model.phi = @(u, theta) ...
        exp(-1i * u * log(L_of_z(theta(2)*theta(4)+theta(4)/2-theta(4)^2/2, theta))) .* ...                 % martingale adjustment
        L_of_z(((u*theta(4)).^2 + 1i * u*theta(4) * (1 + 2 * theta(2))) / 2, theta);         % core CF
